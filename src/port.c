                            ...
                            void __iomem *addr)
{
    ...
    fastcom_set_termination(port, DEFAULT_TERMINATION); /* This does nothing on the 335 cards */
...
int fastcom_get_isosynchronous(struct fc_port *port, int *mode)
{
    switch (fastcom_get_card_type(port)) {
    case CARD_TYPE_FSCC:
...
// TODO
#if 0
void FastcomSetEchoCancelFSCC(struct fc_port *port, int enable)
{
    UINT32 current_fcr, new_fcr;
	UINT32 bit_mask;

    current_fcr = READ_PORT_unsigned long(ULongToPtr(port->Bar2));
    ...
    WRITE_PORT_unsigned long(ULongToPtr(port->Bar2), new_fcr);
}
#endif
...
void fastcom_set_echo_cancel(struct fc_port *port, int enable)
{
    ...
    case CARD_TYPE_FSCC:
        //TODO: fastcom_set_echo_cancel_fscc(port, enable);
        break;
...
int pcie_set_baud_rate(struct fc_port *port, unsigned value)
{
    ...
    dld &= 0xf0;
    dld |= (int)floor(((divisor - floor(divisor)) * port->sample_rate) + 0.5);
...
void fastcom_get_echo_cancel(struct fc_port *port, int *enabled)
{
    ...
    case CARD_TYPE_FSCC:
        //TODO: fastcom_get_echo_cancel_fscc(port, enabled);
        break;
